...
# ---------------- QUIZ ----------------
    ...
    q1 = st.radio("1️⃣ Onde foi nosso primeiro encontro?",
                  ["Cinema","Restaurante","Shopping","Alagoa"])

    if q1 == "Shopping":
        pontos += 1

    q2 = st.radio("2️⃣ Qual foi a primeira coisa que fiz ao te ver?",
                  ["Nada","Sai Correndo","Te Beijei","Cumprimentei"])

    if q2 == "Te Beijei":
        pontos += 1

    q3 = st.radio("3️⃣ Qual comida mais comemos juntos?",
                  ["Lasanha","Sushi","Hambúrguer","Açaí"])

    if q3 == "Açaí":
        pontos += 1

    q4 = st.radio("4️⃣ Onde queremos ver o nascer do Sol?",
                  ["Praça","Praia","Casa","Rua"])

    if q4 == "Praia":
        pontos += 1

    q5 = st.radio("5️⃣ Quem manda mais no relacionamento?",
                  ["Wemilly","Cláudio","Ninguém","Nós dois"])

    if q5 == "Wemilly":
        pontos += 1

    q6 = st.radio("6️⃣ Quantos anos estamos comemorando agora?",
